...
function checkLengthOfIndex(currentIndex, roles) {
    return currentIndex >= roles.length
...
function castIntentionAutoFail(team_total, mission_id, game_id, mission_num) {
    ...
    for (let i = 0; i < team_total; i++) {
        ...
        currentGames[game_id].missions[mission_num - 1].intentions.push({
            mission_id,
            user_id,
            intention,
        });
    ...
    return db.newMission(game_id).then(ids => {
        return db.getMissions(game_id).then(missions => {
            return db.getRoles(game_id).then(roles => {
                ...
                let hammerInitialIndex =
                    missions.length == 1
                        ? randomFirstHammerIndex(roles)
                        : findIndex(
                              currentGames[game_id].currentRound.leader_id,
                              roles
                          ) + 4;
                let finalHammerIndex = checkLengthOfIndex(
                    hammerInitialIndex,
                    roles
                );
                ...
                currentGames[game_id].currentMission = {
                    id: ids[0],
                    mission_num: missions.length,
                    hammer_id: mission.hammer_id,
                    approved: false,
                };
    ...
    return db.getMissions(game_id).then(missions => {
        ...
        return db.getAllRounds(game_id).then(allRounds => {
            const rounds = allRounds.filter(
                round => round.mission_id == mission_id
            );
            const round_num =
                rounds.length > 0 ? rounds[rounds.length - 1].round_num + 1 : 1;
            return db.getRoles(game_id).then(roles => {
                let missionHammer =
                    currentGames[game_id].currentMission.hammer_id;
                let indexOfHammer = findIndex(missionHammer, roles);
                if (missions.length == 1 && round_num == 1) {
                    leaderIdx = workOutLeaderIndexFromHammer(
                        indexOfHammer,
                        roles
                    );
                } else {
                    let lastLeaderId =
                        allRounds[allRounds.length - 1].leader_id;
                    let lastLeaderIndex = findIndex(lastLeaderId, roles);
                    let nextLeaderIndex = checkLengthOfIndex(
                        lastLeaderIndex + 1,
                        roles
                    );
...
function checkNominations(game_id, round_id) {
    const missionParams =
        currentGames[game_id].missionParams[
            currentGames[game_id].currentMission.mission_num - 1
        ];
...
function checkVotes(game_id, round_id) {
    ...
    return db.getVotes(round_id).then(votes => {
        currentGames[game_id].missions[mission_num - 1].rounds[
            round_num - 1
        ].votes = votes;
...
function checkIntentions(game_id, mission_id) {
    const mission_num = currentGames[game_id].currentMission.mission_num;
    const { team_total, fails_needed } = currentGames[game_id].missionParams[
        mission_num - 1
    ];
    return db.getIntentions(mission_id).then(intentions => {
        ...
        if (intentions.length == team_total) {
            if (countIntentions(intentions, fails_needed))
                return missionSucceeds(game_id, mission_id);
...
function missionFails(game_id, mission_id) {
    ...
    console.log(
        currentGames[game_id].missions[mission_num - 1].outcome,
        'outcome set'
    );
