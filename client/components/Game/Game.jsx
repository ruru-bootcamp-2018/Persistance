...
class Game extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        if (
            ...
            newProps.currentGame.gameStage !== 'voting'
        )
            this.grabVotes(newProps.currentGame.missions);
        if (
            this.state.stage == 'intentions' &&
            newProps.currentGame.gameStage !== 'intentions'
        )
            this.sortIntentions(newProps.currentGame.missions);
        if (
            newProps.currentGame.gameStage == 'goodWin' ||
            newProps.currentGame.gameStage == 'spyWin'
        )
            this.setState({ gameOver: true });
        if (
            this.state.stage == 'nominating' &&
            newProps.currentGame.currentRound.leader_id ==
                newProps.currentGame.currentMission.hammer_id
        )
            this.setState({ showHammerInfo: true });
        if (
            this.state.round.round_num == 5 &&
            newProps.currentMission.mission_approved == false
        )
            this.setState({ hammerFail: true });
    ...
    sortIntentions(missions) {
        ...
        this.setState({
            showIntentions: true,
            mission: { intentions, team, outcome: mission.outcome },
        });
    }
    ...
    render() {
        let hammerPlayer = this.props.currentGame.players.find(
            x => x.id == this.props.currentGame.currentMission.hammer_id
        );
        return (
            <div className="container">
                <StatusBar
                    leader={
                        this.props.currentGame.currentRound.leader_id ==
                ...
                {this.state.gameOver && (
                    <GameOver hideModal={this.hideGameOver.bind(this)} />
                )}
                {this.state.showIntentions && (
                    <IntentionsSuspense
                        hideModal={this.hideModal.bind(this)}
                        mission={this.state.mission}
                    />
                )}
                {this.state.showHammerInfo && (
                    <Hammer
                        hideModal={this.hideModal.bind(this)}
                        hammer={hammerPlayer}
                    />
                )}
                {this.state.hammerFail && (
                    <HammerFail hideModal={this.hideModal.bind(this)} />
                )}
