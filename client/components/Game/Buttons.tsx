...
const Buttons = () => {
    ...
    const isLeader = useSelector(
        state => state.currentGame.currentRound.leader_id == authId
    );
    ...
    const intentions = useSelector(
        state => state.currentGame.missions[mission_num - 1].intentions
    );
    ...
    return (
        <div>
            ...
            {onTeam && gameStage === 'intentions' && intentions.length < 0 && (
                <IntentionButtons />
            )}
            {gameStage === 'nominating' && isLeader && allNoms && (
                <ConfirmNom />
            )}
        </div>
    );
