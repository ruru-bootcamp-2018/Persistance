...
class Player extends React.Component {
    ...
    handleClick() {
        ...
        if (!this.state.isNominated) {
            ...
            sendNomination(nom).then(res => {
                ...
                this.setState({
                    isNominated: true,
                    roundId: this.props.currentGame.currentRound.id,
                });
            });
        } else {
    ...
    componentWillReceiveProps(nextProps) {
        if (
            ...
            nextProps.currentGame.currentRound.id != this.state.roundId
        )
        ...
        const nominations =
            nextProps.currentGame.missions[mission_num - 1].rounds[
                round_num - 1
            ].nominations;
        const nominatedUser = nominations.find(
            player => player.user_id == nextProps.player.id
        );
        if (nominatedUser && this.state.isNominated == false)
    ...
    render() {
        ...
        const currentUser = this.props.currentGame.players.find(
            player => player.id == id
        );
        ...
        const isNominating =
            this.props.leader == authID &&
            this.props.currentGame.gameStage == 'nominating';
        ...
        const isNominated = noms.some(
            nom => nom.user_id == this.props.player.id
        );
        ...
        return (
            <div>
                ...
                >
                    <div
                        onClick={isNominating && this.handleClick}
                        className="player"
                    >
